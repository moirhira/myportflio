...
/**
 * Projects component (Tailwind version)
 *
 * - Fetches /projects.json (place the file in your public/ folder).
 * - Shows loading / error / empty states.
 * - Uses Tailwind utility classes and the small custom helpers from your index.css
 *   (e.g. .project-card-custom) but can work without them if you prefer pure Tailwind.
 *
 * If you already have a ProjectCard component, you can replace the inner markup with that component.
 */
export default function Projects() {
  ...
  return (
    <div className="projects-container mx-auto w-11/12 max-w-1600 px-5 py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project) => (
          <article
            key={project.id}
            className="project-card-custom flex flex-col overflow-hidden"
          ...
          >
            ...
            <div className="project-info p-6 flex flex-col flex-1">
              ...
              <h3 id={`project-${project.id}-title`} className="text-white text-xl mb-3">
                {project.title}
              </h3>
              <p className="text-[color:var(--light-text)] opacity-90 mb-4 flex-1">
                {project.description}
              </p>
